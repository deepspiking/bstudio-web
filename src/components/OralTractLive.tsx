...
const MODES = {
  ...
  [MODE_LOW_BAL]: { view: 'ecapa/lang2d_low_bal_05vad', encoder: 'ecapa', label: 'ECAPA LDA · LOW+en확장 (0.5s·VAD)' },
} as const
const XMIN = -1.12
const XMAX = 1.12
const PAD_X = 52
...
export default function OralTractLive() {
  ...
  const [mode, setMode] = useState<string>(MODE_100)
  ...
  const viewKeyRef = useRef(MODES[mode as keyof typeof MODES].view)
  viewKeyRef.current = MODES[mode as keyof typeof MODES].view
  const encoderRef = useRef(MODES[mode as keyof typeof MODES].encoder)
  encoderRef.current = MODES[mode as keyof typeof MODES].encoder
  ...
  const finishCapture = useCallback(() => {
    ...
    for (const run of runs) {
      ...
      segments.push({
        ...
        frames: run.map((f) => ({ ...f, t: Math.max(0, f.t - t0 - GRAPH_SHIFT_SEC) })),
